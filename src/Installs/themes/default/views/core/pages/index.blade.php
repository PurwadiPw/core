...
@section("contentheader_description", "Pages Editor")
@section("section", "Pages")
@section("sub_section", "Editor")
@section("htmlheader_title", "Pages Editor")
                    ...
                    <ul id="widget-tab-1" class="nav nav-tabs">
                        ...
                        <li class="active">
                            <a data-toggle="tab" data-act="act-list" href="#frm-list"> 
                                ...
                                <span class="hidden-mobile hidden-tablet"> Pages </span> 
                            </a>
                        </li>
                    ...
                    <div class="widget-body no-padding">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade in active" id="frm-list">
                                @include('default::core.pages.opr.list')
                            </div>
...
<script>
    ...
    var table = $('#dt_ajax').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "{{ url(config('core.adminRoute').'/core_pages_dt_ajax') }}",
    ...
    $('#action-row a').on('click', function(e){
        ...
        if (row.length > 1) {
            ...
            var data = {
                '_token': "{{ csrf_token() }}",
                'id': id,
                'url': "{{ route(config('core.adminRoute').'.core_pages.destroy', "+id+") }}",
                'count': row.length,
                'tbl': '#dt_ajax'
            };
            delMultiple(data);
        ...
        var data = {
            'token': "{{ csrf_token() }}",
            'dataId': id,
            'url': "{{ url(config('core.adminRoute').'/core_pages/openTab') }}",
        };
        openTab(act, id, data);
