		...
		if(Crud::hasAccess("Users", "view")) {
			...
			if(isset($user->id)) {
				if($user['type'] == "Employee") {
					return redirect(config('core.adminRoute') . '/employees/'.$user->id);
				} else if($user['type'] == "Client") {
					return redirect(config('core.adminRoute') . '/clients/'.$user->id);
				}
			} else {
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('core.adminRoute') . '/users/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				...
				if(Crud::hasAccess("Users", "edit")) {
					$output .= '<a href="'.url(config('core.adminRoute') . '/users/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
				
				if(Crud::hasAccess("Users", "delete")) {
					$output .= Form::open(['route' => [config('core.adminRoute') . '.users.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
