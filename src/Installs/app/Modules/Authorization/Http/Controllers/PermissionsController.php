...
class PermissionsController extends Controller
{
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url('authorization/permissions/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Crud::hasAccess("Permissions", "edit")) {
					$output .= '<a href="'.url('authorization/permissions/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
				
				if(Crud::hasAccess("Permissions", "delete")) {
					$output .= Form::open(['route' => ['authorization.permissions.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
	...
	public function save_permissions(Request $request, $id)
	{
		if(Entrust::hasRole('SUPER_ADMIN')) {
			...
			return redirect('authorization/permissions/'.$id."#tab-access");
		} else {
			return redirect(config('core.adminRoute')."/");
		}
	}
