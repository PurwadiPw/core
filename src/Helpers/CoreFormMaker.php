...
class CoreFormMaker
{
     ...
     **/
    public static function input($crud, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
    {
        // Check Field Write Aceess
        if(Crud::hasFieldAccess($crud->id, $crud->fields[$field_name]['id'], $access_type = "write")) {
            ...
            //print_r($crud->fields);
            $label = $crud->fields[$field_name]['label'];
            $field_type = $crud->fields[$field_name]['field_type'];
            $unique = $crud->fields[$field_name]['unique'];
            $defaultvalue = $crud->fields[$field_name]['defaultvalue'];
            $minlength = $crud->fields[$field_name]['minlength'];
            $maxlength = $crud->fields[$field_name]['maxlength'];
            $required = $crud->fields[$field_name]['required'];
            $popup_vals = $crud->fields[$field_name]['popup_vals'];

            if($required2 != null) {
                $required = $required2;
            }

            $field_type = CrudFieldTypes::find($field_type);
            ...
            if($unique && !isset($params['unique'])) {
                ...
                $params['field_id'] = $crud->fields[$field_name]['id'];
                $params['adminRoute'] = config('core.adminRoute');
                if(isset($row)) {
                    $params['isEdit'] = true;
                ...
                $out .= '<input type="hidden" name="_token_'.$crud->fields[$field_name]['id'].'" value="'.csrf_token().'">';
            }
            ...
            switch ($field_type->name) {
                ...
                case 'Dropdown':
                    ...
                    if($popup_vals != "") {
                        $popup_vals = CoreFormMaker::process_values($popup_vals);
                    } else {
                        $popup_vals = array();
                    }
                ...
                case 'File':
                    ...
                    if($default_val != 0) {
                        $upload = \App\Modules\Content\Models\Upload::find($default_val);
                    }
                    if(isset($upload->id)) {
                ...
                case 'Files':
                    ...
                    if(is_array($default_val_arr) && count($default_val_arr) > 0) {
                        ...
                        foreach ($default_val_arr as $uploadId) {
                            $upload = \App\Modules\Content\Models\Upload::find($uploadId);
                            if(isset($upload->id)) {
                                $uploadIds[] = $upload->id;
                                $fileImage = "";
                ...
                case 'Taginput':
                    ...
                    $default_val = CoreFormMaker::process_values($default_val);
     ...
     * get data from crud / table whichever is found if starts with '@'
     **/
    // $values = CoreFormMaker::process_values($data);
    ...
    /**
     * Display field using blade directive @core_display
     **/
    public static function display($crud, $field_name, $class = 'form-control')
    {
        // Check Field View Aceess
        if(Crud::hasFieldAccess($crud->id, $crud->fields[$field_name]['id'], $access_type = "view")) {

            $fieldObj = $crud->fields[$field_name];
            $label = $crud->fields[$field_name]['label'];
            $field_type = $crud->fields[$field_name]['field_type'];
            $field_type = CrudFieldTypes::find($field_type);
            ...
            switch ($field_type->name) {
                ...
                case 'Decimal':
                    ...
                    break;
                case 'Dropdown':
                    $values = CoreFormMaker::process_values($fieldObj['popup_vals']);
                    if(starts_with($fieldObj['popup_vals'], "@")) {
                        if($value != 0) {
                            $crudVal = Crud::getByTable(str_replace("@", "", $fieldObj['popup_vals']));
                            if(isset($crudVal->id)) {
                                $value = "<a href='".url(config("core.adminRoute")."/".$crudVal->name_db."/".$value)."' class='label label-primary'>".$values[$value]."</a> ";
                            } else {
                ...
                case 'File':
                    if($value != 0) {
                        $upload = \App\Modules\Content\Models\Upload::find($value);
                        if(isset($upload->id)) {
                        ...
                        if(starts_with($fieldObj['popup_vals'], "@")) {
                            ...
                            foreach ($values as $key => $val) {
                                if(in_array($key, $valueSel)) {
                                    $valueOut .= "<a href='".url(config("core.adminRoute")."/".$crudVal->name_db."/".$key)."' class='label label-primary'>".$val."</a> ";
                                }
                            }
                        } else {
                            $valueSel = json_decode($value);
    ...
    /**
     * Print form using blade directive @core_form
     **/
    public static function form($crud, $fields = [])
    {
        if(count($fields) == 0) {
            $fields = array_keys($crud->fields);
        }
        $out = "";
        foreach ($fields as $field) {
            $out .= CoreFormMaker::input($crud, $field);
        }
        return $out;
    }
     ...
     **/
    public static function core_access($crud_id, $access_type = "view", $user_id = 0)
    {
        return Crud::hasAccess($crud_id, $access_type, $user_id);
    }

    /**
     * Check Whether User has Module Field Access
     **/
    public static function core_field_access($crud_id, $field_id, $access_type = "view", $user_id = 0)
    {
        return Crud::hasFieldAccess($crud_id, $field_id, $access_type, $user_id);
    }
}